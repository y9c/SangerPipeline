...
def get_options():
    ...
    o.add_option('--map', dest='corresponding_table', help='table file tha Map sequence, primer and reference')
    o.add_option('--company', dest='company_format', type='choice', default='invitrogen', choices=['invitrogen', 'bgi', 'generay', 'genewiz'])
...
def parse_ab1(ab1_file):
    with open(ab1_file, 'rb') as handle:
        seq = SeqIO.parse(handle, 'abi').next()
    return seq
        ...
        for name in files:
            ...
            if name[-4:] == ".abl" or name[-4:] == ".ab1" or name[-4:] == ".abi":
                if company_format == 'invitrogen' and len(name.split('.'))>5:
                    ...
                    if group_dic_of_file_path.has_key(ref_name):
...
def align( file_name_raw, subject_fasta, company_format, corresponding_table, ignore_ambig):
    folder_name = decomplessing_file(file_name_raw)
    group_dic_of_file_path = classify(folder_name, company_format, corresponding_table)
    ...
    for ref_name in group_dic_of_file_path:
        ...
        for file_path in group_dic_of_file_path[ref_name]:
            ...
            print "the {}th file :----------->\n{}& {}".format(count, ref_name, file_path)
            count += 1
            mutations = []
            mutations, r1 = tcoffee_align( ref_sequence, query_seq, ignore_ambig )
            for m in mutations:
                print m

# main part of the program
def main( options ):
    align( options.file_name_raw, options.subject_fasta, options.company_format, options.corresponding_table, options.ignore_ambig)
